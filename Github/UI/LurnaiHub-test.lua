...
task.spawn(function()
	pcall(function()
		...
		loadstring(game:HttpGet("https://raw.githubusercontent.com/HVX-Havoc/Lurnai-Hub/refs/heads/main/Github/UI/Lurnai_Discord_Ad.Lua"))()
	end)
end)
...
local Themes = {
    Names = {
        "LurnaiHub", "Green", "Blue", "LightBlue", "Gray", "Red", "Orange", "Purple", "Pink", "Teal"
    },
    LurnaiHub = {
        Name = "LurnaiHub",
...
ElementsTable.Folder = (function()
    local Element = {}
    Element.__index = Element
    Element.__type = "Folder"

    function Element:New(Config)
        assert(Config.Title, "Folder - Missing Title")
        
        local Folder = {
            Opened = Config.Default or false,
            Elements = {},
            Type = "Folder"
        }
        
        -- Create main folder button/container
        local FolderFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
        FolderFrame.DescLabel.Size = UDim2.new(1, -54, 0, 14)
        
        Folder.SetTitle = FolderFrame.SetTitle
        Folder.SetDesc = FolderFrame.SetDesc
        Folder.Visible = FolderFrame.Visible
        Folder.Container = FolderFrame
        
        -- Create the arrow indicator
        local FolderArrow = New("ImageLabel", {
            AnchorPoint = Vector2.new(1, 0.5),
            Size = UDim2.fromOffset(16, 16),
            Position = UDim2.new(1, -10, 0.5, 0),
            BackgroundTransparency = 1,
            Image = "rbxassetid://10709791281", -- Chevron icon
            Parent = FolderFrame.Frame,
            ThemeTag = {
                ImageColor3 = "Text",
            },
        })
        ...
        local ElementContainer = New("Frame", {
            ...
            New("UIPadding", {
                PaddingLeft = UDim.new(0, 15), -- Indentation for folder contents
            })
        })
        
        -- Update folder visual state
        function Folder:UpdateState()
            if self.Opened then
                ...
                TweenService:Create(
                    ElementContainer, 
                    TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
                    {Size = UDim2.new(1, 0, 0, ElementContainer.UIListLayout.AbsoluteContentSize.Y)}
                ):Play()
            else
                TweenService:Create(
                    FolderArrow, 
        ...
        -- Set folder state
        function Folder:SetState(state)
            self.Opened = state
            self:UpdateState()
        end
        
        -- Update layout when elements change
        Creator.AddSignal(ElementContainer.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
            if Folder.Opened then
                ElementContainer.Size = UDim2.new(1, 0, 0, ElementContainer.UIListLayout.AbsoluteContentSize.Y)
            end
        end)
        
        -- Handle click to expand/collapse
        Creator.AddSignal(FolderFrame.Frame.MouseButton1Click, function()
            Folder:Toggle()
        end)
        
        -- Initialize folder state
        Folder:SetState(Folder.Opened)
        
        -- Add element creation functions for the folder content
        setmetatable(Folder, Elements)
        Folder.Container = ElementContainer
        Folder.ScrollFrame = self.ScrollFrame
